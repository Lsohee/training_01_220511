...
// todo: html 문서에 stepCounterOneBone 이라는 js 파일이 먼저 설치되어있는 것을 확인 
// todo: 결과적으로 해당 파일과 위의 파일이 연결된것 : 모듈화 -> 연결 


// ^ HTML에 직접쓴 var count = new Countstep 이 가져오는 역할을 했나봐
// ? 왜 도대체 script를 따로 단거임? 두개가 다른 언어로 쓰여졌나?

// todo : 두개를 분리해서 관리하는 것, 자바스크립트의 코드 량이 늘어날 수록 잘게 잘라서 관리함

(function(){
	// ? 함수가 맞긴한가? 왜 소괄호만 두개 연달아서 쓰지?

	// todo : 즉시실행함수 유형으로 선언, 호출 따로 하지 않고 선언하면서 동시에 호출도 하겠다는 작성법

	// 함수가 맞으면 편의상 First 라고 부르겠음

	function Countstep(){ 

		// todo : Countstep() 이라는 함수는 첫글자가 대문자 = 생성자 함수 라는 것을 의미하며, 객체를 찍어내는 거푸집 본체와 같음 객체를 생성하는 틀이라고 볼 수 있으며, 이것은 알고리즘 중 '다량의 객체'를 편하게 만들게 하기 위해 마련된 작성법이므로, 객체를 많이 사용해야할때 '생성자 함수' 를 학습할 것을 권하며 이것에 대한 다른 문법도 존재함

		// todo : 자바스크립트에서 부르는 class 가 따로 있는데 이것은 css에서 사용하는 class와 다른 의미임
		// todo : 회원가입 기능이 대표적인 '객체'를 복사해서 사용하는 방식이고 위의 Countstep() 이 객체의 본체
...
// todo : 총답변 해당 파일은 밥을 만들때 사용하는 쌀알을 어떻게 조리하느냐를 다루었다기보다는 밥솥을 이야기 하는 것이기때문에 기반 사항이 많이 필요함 가볍게 눈으로 읽어보는 용도로 흥미를 잃지 않게 확인할 것을 권하며

// todo : 해당 파일을 깊게 파볼 시점은 '객체'를 많이 만들어야 할때, 유지보수를 위해 코드를 관리해야할때 유용한 방식
